...
@section('content')
    ...
    <div class="container-fluid mt-3 p-0">
        <div class="row mt-3"
            style="background-image: linear-gradient(rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.6)), url('{{ asset('Frontend/Home/assets/imgs/aub25.jpg') }}'); background-size: cover; background-repeat: no-repeat; background-position: center; height: 140vh; display: flex; align-items: center; justify-content: space-between; padding: 20px; color: #fff; ">
                ...
                <div class="containair" >
                    <div class="row d-flex align-items-center justify-content-center" style="height: 80vh;">
                        ...
                        <div class="col-md-6 p-5" style="position: relative; top: -200px; background-color: #fff;  border-radius: 10px;box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.1);">
                            ...
                            <form action="{{ route('signup.user') }}" method="POST" class="mt-4">
                                @csrf
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select name="role" id="role" class="form-control"
                                        style="border: 1px solid #ccc; border-radius: 5px; padding: 10px;">
                                        <option value="VENDEUR" {{ old('role') == 'VENDEUR' ? 'selected' : '' }}>Vendeur
                                        </option>
                                        <option value="AGENCE" {{ old('role') == 'AGENCE' ? 'selected' : '' }}>Agence
                                        </option>
                                        <option value="ADMIN" {{ old('role') == 'ADMIN' ? 'selected' : '' }}>ADMIN
                                        </option>
                                        <option value="USER" {{ old('role') == 'USER' ? 'selected' : '' }}>Simple
                                            Utilisateur</option>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <textarea name="adresse" id="adresse" class="form-control" rows="3"
                                        style="border: 1px solid #ccc; border-radius: 5px; padding: 10px;">{{ old('adresse') }}</textarea>
                                    <span class="text-danger">
                                        @error('adresse')
                                            {{ $message }}
                                        @enderror
                                ...
                                <div class="text-center w-100 mt-3" style="font-size: 25px">
                                    <p class="text-muted font-weight-bold">Vous avez deja un compte ?
                                        <a href="{{ route('loginPage') }}" class="ml-2" style="color:#318093;">Se
                                            connecter</a>
                                    </p>
                                </div>
                            </form>
