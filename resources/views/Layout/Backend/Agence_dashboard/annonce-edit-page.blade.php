...
<div class="container">
    ...
    <form action="{{ route('annonce.update', $annonce->annonce_id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control" id="montant" name="montant" value="{{ old('montant', $annonce->montant) }}" required min="0">
        </div>

        <div class="form-group">
            <label for="typePropriete">Type de Propriété</label>
            <select class="form-control" id="typePropriete" name="typePropriete" required>
                <option value="Appartement" {{ $annonce->typePropriete == 'Appartement' ? 'selected' : '' }}>Appartement</option>
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control" id="superficie" name="superficie" value="{{ old('superficie', $annonce->superficie) }}" min="0">
        </div>

        <div class="form-group">
            <label for="nbChambres">Nombre de Chambres</label>
            <input type="number" class="form-control" id="nbChambres" name="nbChambres" value="{{ old('nbChambres', $annonce->nbChambres) }}" min="0">
        </div>

        <div class="form-group">
            <label for="nbSalleDeDouche">Nombre de Salles de Douche</label>
            <input type="number" class="form-control" id="nbSalleDeDouche" name="nbSalleDeDouche" value="{{ old('nbSalleDeDouche', $annonce->nbSalleDeDouche) }}" min="0">
        </div>

        <div class="form-group">
            <label for="localite">Localité</label>
            <input type="text" class="form-control" id="localite" name="localite" value="{{ old('localite', $annonce->localite) }}" required>
        </div>

        <div class="form-group">
            <label for="localisation">Localisation (Adresse)</label>
            <input type="text" class="form-control" id="localisation" name="localisation" value="{{ old('localisation', $annonce->localisation) }}">
        </div>
        ...
        <div class="form-group">
            ...
            <select class="form-control" id="typeTransaction" name="typeTransaction" required>
                <option value="A vendre" {{ $annonce->typeTransaction == 'A vendre' ? 'selected' : '' }}>A vendre</option>
        ...
        <div class="form-group">
            ...
            <input type="url" class="form-control" id="visite360" name="visite360" value="{{ old('visite360', $annonce->visite360) }}">
        </div>

        <div class="form-group">
            <label for="video">Vidéo (URL)</label>
            <input type="url" class="form-control" id="video" name="video" value="{{ old('video', $annonce->video) }}">
        </div>
